...
class SubfinderJob implements ShouldQueue
{
    ...
    public function handle(ReconService $recon, ToolsClientService $tools): void
    {
        $run = MetisJobRun::findOrFail($this->jobRunId);

        try {
            $recon->subfinder($run, $tools);
        } catch (\Throwable $e) {
            Log::error("SubfinderJob [{$this->jobRunId}] failed: " . $e->getMessage());
